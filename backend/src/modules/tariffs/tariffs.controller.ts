...
export class TariffsController { constructor(private readonly service: TariffsService) {}
  ...
  @Patch(':id') @Roles(RoleName.ADMIN) update(@Param('id') id:number,@Body() d:UpdateTariffDto,@CurrentUser() u:AuthUser,@Req() r:Request){return this.service.update(id,d,u.id,r.ip)}
  @Post(':id/activate') @Roles(RoleName.ADMIN) activate(@Param('id') id:number,@CurrentUser() u:AuthUser,@Req() r:Request){return this.service.activate(id,u.id,r.ip)}
  @Post(':id/retire') @Roles(RoleName.ADMIN) retire(@Param('id') id:number,@CurrentUser() u:AuthUser,@Req() r:Request){return this.service.retire(id,u.id,r.ip)}
}
