...
export class TariffsService {
  ...
  async findOne(id: number) { const value = await this.prisma.tariff.findUnique({ where: { id } }); if (!value) throw new NotFoundException('Tarifa no encontrada'); return value; }
  async create(dto: CreateTariffDto, userId: number, ipAddress?: string) {
  ...
  async update(id: number, dto: UpdateTariffDto, userId: number, ipAddress?: string) {
  ...
  async activate(id: number, userId: number, ipAddress?: string) {
  ...
  async retire(id: number, userId: number, ipAddress?: string) { const current = await this.findOne(id); if (current.status !== TariffStatus.ACTIVE) throw new UnprocessableEntityException('Solo se puede retirar una tarifa activa'); return this.change(id, { status: TariffStatus.RETIRED }, current, userId, ipAddress); }
  ...
  private change(id: number, data: Prisma.TariffUpdateInput, before: object, userId: number, ipAddress?: string) { return this.prisma.$transaction(async (tx) => { const value = await tx.tariff.update({ where: { id }, data }); await this.audit.log({ userId, action: AuditAction.UPDATE, entity: AuditEntity.TARIFF, entityId: id, changes: { before, after: value }, ipAddress }, tx); return value; }); }
}
