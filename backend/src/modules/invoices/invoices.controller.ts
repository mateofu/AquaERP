import { Body, Controller, Get, Param, ParseUUIDPipe, Post, Query, Req, UseGuards } from '@nestjs/common'; import { ApiBearerAuth, ApiOperation, ApiTags } from '@nestjs/swagger'; import { RoleName } from '@prisma/client'; import { Request } from 'express';
...
export class InvoicesController {constructor(private readonly service:InvoicesService){}
  ...
  @Get('eligible-readings/:billingPeriodId') @Roles(...WRITE) async eligible(@Param('billingPeriodId',ParseUUIDPipe) id:string){return{data:await this.service.eligibleReadings(id)}}
  ...
  @Post(':id/issue') @Roles(...WRITE) issue(@Param('id') id:string,@CurrentUser() u:AuthUser,@Req() r:Request){return this.service.issue(id,u.id,r.ip)}
  @Post(':id/void') @Roles(RoleName.ADMIN) void(@Param('id') id:string,@Body() d:VoidInvoiceDto,@CurrentUser() u:AuthUser,@Req() r:Request){return this.service.void(id,d.reason,u.id,r.ip)}
}
