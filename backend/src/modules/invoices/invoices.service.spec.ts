...
describe('InvoicesService', () => {
  const reading = {
    id: 'reading-1', meterId: 'meter-1', billingPeriodId: 'period-1',
    readingValue: new Prisma.Decimal(25), previousValue: new Prisma.Decimal(15),
    consumption: new Prisma.Decimal(10),
    billingPeriod: { id: 'period-1', year: 2026, month: 7, status: BillingPeriodStatus.CLOSED },
    meter: { serialNumber: 'M-001', property: { code: 'P-001', address: 'Vereda Centro', customer: { id: 'customer-1', firstName: 'Ana', lastName: 'Rural', documentNumber: '123' } } },
  };
  const tariff = { id: 'tariff-1', name: 'General', status: TariffStatus.ACTIVE, fixedCharge: new Prisma.Decimal(15000), pricePerCubicMeter: new Prisma.Decimal(1800) };
  let tx: { meterReading: { findUnique: jest.Mock }; tariff: { findFirst: jest.Mock }; invoice: { create: jest.Mock; update: jest.Mock } };
  let service: InvoicesService;

  beforeEach(() => {
    tx = { meterReading: { findUnique: jest.fn().mockResolvedValue(reading) }, tariff: { findFirst: jest.fn().mockResolvedValue(tariff) }, invoice: { create: jest.fn().mockImplementation(({ data }: { data: Prisma.InvoiceCreateInput }) => Promise.resolve({ id: 'invoice-1', status: InvoiceStatus.DRAFT, ...data, items: [] })), update: jest.fn() } };
  ...
  it('calcula cargo fijo, consumo y total usando valores decimales', async () => {
    const result = await service.generate({ meterReadingId: 'reading-1', issueDate: '2026-07-31', dueDate: '2026-08-15' }, 'user-1');
  ...
  it('rechaza lecturas de periodos que no están cerrados', async () => {
    ...
    await expect(service.generate({ meterReadingId: 'reading-1' }, 'user-1')).rejects.toBeInstanceOf(UnprocessableEntityException);
  });
  ...
  it('traduce una restricción única en conflicto de negocio', async () => {
    ...
    await expect(service.generate({ meterReadingId: 'reading-1' }, 'user-1')).rejects.toBeInstanceOf(ConflictException);
  });
});
