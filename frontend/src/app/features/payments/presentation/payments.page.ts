...
export class PaymentsPage implements OnInit {
  readonly facade = inject(PaymentsFacade); private readonly session = inject(AuthSessionService); private readonly unsavedChanges = inject(UnsavedChangesService); private readonly excel = inject(ExcelExportService);
  ...
  exportExcel(): void { this.excel.download('payments', 'pagos-y-cartera', { search: this.facade.search() }).subscribe(); }
